...
options(prompt = "R> ", continue = "+  ", width = 68, useFancyQuotes = FALSE)
@

%for JSS style
\usepackage{setspace}
\definecolor{fgcolor}{rgb}{0,0,0}
\renewenvironment{knitrout}{\vspace{-2mm}\begin{onehalfspacing}}{\end{onehalfspacing}\vspace{-2mm}}
...
%Title
\title{The \pkg{jsonlite} Package: A Practical and Consistent Mapping Between \JSON Data and \R Objects}
\Plaintitle{The jsonlite Package: A Practical and Consistent Mapping Between JSON Data and R Objects}
\Shorttitle{The \pkg{jsonlite} Package}
...
\Keywords{JSON, data-interchange}
\Plainkeywords{JSON, data-interchange}
...
%references
\bibliography{../vignettes/references}
