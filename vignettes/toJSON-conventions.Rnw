...
\noindent This leads us to the following convention: when encoding R objects, \JSON primitives only appear in vectors and data-frame rows. Primitive values within a \JSON array indicate a vector, and \JSON primitives appearing inside a \JSON object indicate a data-frame row. A \JSON encoded \texttt{list}, (named or unnamed) will never contain \JSON primitives. This is a subtle but important convention that allows us to distinguish between R classes from their \JSON resprestation, without explicitly encoding any metadata.

\subsubsection{Missing values in data frames}

The section on atomic vectors discussed two methods of encoding missing data appearing in a vector: either using strings or using the native \texttt{null} type. When a missing value appears in a data frame, there is a third option: simply not include this field in \JSON record:
...
\subsubsection{Nested lists within records}

Something that is a bit unusual in R, but fairly common in \JSON, are records within a data set containing nested lists. Even though not often used, the data frame class in R does allow for a lists columns as well as vector columns, as long as they are all the same length. Because such structures are very common in \JSON, the JSONlite package supports this as well:

<<tidy=FALSE>>=
x <- data.frame(driver=c("Bowser","Peach"), job=c("Koopa","Princess"))
...
Watch out for imbalanced records

<<tidy=FALSE>>=
x <- data.frame(name=c("Elizabeth", "Jeroen"), age=c(24, 28));
x$siblings <- list(
  data.frame(name="Ricardo", age=27), data.frame(name=c("Erik", "Marcel"), age=c(27, 25))
)
cat(toJSON(x, pretty=TRUE))
@
