...
\noindent This leads us to the following convention: when encoding R objects, \JSON primitives are only used for encoding vectors and data-frame rows. Primitives appearing within a \JSON array indicate a vector, and \JSON arrays appearing inside a \JSON object indicate a data-frame row. A \JSON encoded \texttt{list}, (named or unnamed) will never contain \JSON primitives. This is a subtle but important convention that helps to distinguish between R classes from their \JSON resprestation.

\subsubsection{Missing values in data frames}

The section on atomic vectors discussed two methods of encoding missing data appearing in a vector: either using strings or using the native \texttt{null} type. When a missing value appears in a data frame, there is a third option: simply not include this field in the record at hand.
...
\noindent The default behavior of JSONlite is to omit missing data from records in a data frame. This seems to be the most conventional way of doing things, and we expect is most likely to lead to the correct interpretation of \emph{missingness}, even in languages which have no explicit notion of \texttt{NA}.
