...
It can be helpful to see the problem from both sides. The R user would like to interact with external \JSON data/interfaces from within R. This includes reading or posting \JSON data from/to a public API or 3rd party service. From perspective of the R user, \JSON data should be converted to the corresponding R classes which are most natural for a particular structure. This makes it easier for the R user to read/generate dynamic \JSON data as provided/required by the external source. Ideally, the R user would like to forget about the interchange format at all, and think about the external data interface in terms of its corresponding R structures. The other perspective is that of an external client or language, which would like to access and manipulate R objects through their \JSON representation. The mapping between \JSON and R objects should allow 3rd party clients to get data in and out of R, without necessarily understanding the specifics of R classes. Ideally, the external client could forget about the R objects and classes at all, and think about input and output of data completely in terms of the \JSON structures or the corresponding realization in the language of the client. 
