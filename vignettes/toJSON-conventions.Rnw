...
\subsubsection{Nested objects within records}
...
\noindent The case of nested subrecords is relatively well behaved. They map nicely to nested dataframes, and if we wish to do so they can easily be flattened into a basic non-nested dataframe for easier manipulation in R:

<<>>=
myjson <- toJSON(x)
mydata <- fromJSON(myjson, flatten=TRUE);
str(mydata)
@

\noindent Things get more tricky when records include nested arrays.  
