...
We can roughly distinguish two use cases. The first one is interacting with 3rd party \JSON data sources from within R. This will mostly involve reading dynamic \JSON data from public sources or APIs. 



The main goal of JSONlite is to define a consistent, natural and practical mapping between \JSON structures and R objects. This is useful for interoperability, both when R is a server or a client role. From the perspective of the R user, a consistent mapping makes it easy to read, write and manipulate a JSON structure using the corresponding object. This is useful, for example when consuming live/dynamic data from a public \JSON API in R. In order for R code to work, the \JSON data must consistently be converted to the same R classes for all possible outputs of the API.

From the perspective of a client, it allows the client to access and manipulate R objects through their \JSON representation. This allows clients to get data in and out of R, for example in the context of an RPC serverice such as OpenCPU.
...
\begin{itemize}
  ...
  \item{Good support for \JSON from 3rd party API.}
  \item{Interact with R in RPC setting.}
  \item{Consistent behavior to build on systems with dynamic data}
  \item{Optimally reversible, at least coercible.}
  \item{Minimize opportunities for misinterpretation.}
\end{itemize}

Much of this work has been motived by the need to support real world API's and \emph{dynamic data}.tgm
...
In the design of the functions we target consistent behavior, and try to adhere to the \emph{robustness principle} to be strict on output but tolerant on input. 

\section{Converting R objects to \JSON}
 
This section lists options and examples of representing the common R classes in \JSON, i.e. the \toJSON function in \texttt{JSONlite}. As explained before, we rely on the class based S3 method system in R; hence objects get encoded according to their \texttt{class} value. If an object has multiple \texttt{class} values, R will use the first occurring class which has a \toJSON method implemented. If none of the classes of an object has a \toJSON method, R will raise an error.
