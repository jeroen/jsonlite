...
\noindent The case of nested subrecords is relatively well behaved. They map nicely to nested dataframes, and can be flattened into a basic non-nested dataframe for easier manipulation. Building on the previous example:
...
\noindent Things get difficult when records include nested arrays. It works but we can't really flatten them (test this!!)

<<>>=
x <- data.frame(name=c("Elizabeth", "Jeroen"), age=c(24, 28));
x$siblings <- list("ricardo", c("Erik", "Marcel"))
cat(toJSON(x, pretty=TRUE))
fromJSON(toJSON(x))
@

Something else we commonly see is a field containing an entire new dataset (a set of records):
