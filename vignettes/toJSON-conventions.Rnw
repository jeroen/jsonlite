...
The main goal of JSONlite is to define a consistent and practical mapping between \JSON structures and R objects such that it becomes easy to read, write and manipulate a JSON structure in R using the corresponding object, and vice versa. Two important use cases include consuming live/dynamic data from public API's and getting data in and out of R in the context of an RPC service such as OpenCPU. 
