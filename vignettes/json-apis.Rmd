...
Below an example from the [ProPublica Nonprofit Explorer API](http://projects.propublica.org/nonprofits/api) where we retrieve the first 10 pages of tax-exempt organizations in the USA, ordered by revenue. The `plyr` package is used to combine the pages into a single data frame. See also the [jsonlite paging vignette](http://cran.r-project.org/web/packages/jsonlite/vignettes/json-paging.html).
...
print(bestsellers$display_name)
