...
%\VignetteIndexEntry{Examples of using jsonlite with public JSON APIs}
...
This section lists some examples of public \texttt{HTTP} \API'ss that publish data in \JSON format. These are great to get a sense of the complex structures that are encountered in real world \JSON data. All services are free, but some require registration/authentication.
