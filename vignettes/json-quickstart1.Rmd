...
%\VignetteIndexEntry{Getting started 1: Parsing JSON with jsonlite}
-->
...
| Argument          | JSON structure               | R class       |
| ------------------|------------------------------|----------------
| simplifyVector    | Array of primitives          | Atomic Vector |
| simplifyDataFrame | Array of objects             | Data Frame    |
| simplifyMatrix    | Array of equal length arrays | Matrix        |
...
The data frame can be converted back into the original JSON structure using `toJSON` (whitespace and linebreaks are ignorable in JSON).
