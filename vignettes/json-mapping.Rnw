...
\abstract{The native R realization of \JSON data types is a \texttt{named list} for \JSON objects, and \texttt{unnamed list} for \JSON arrays. However, in practice a list is an awkward, inefficient type to store and manipulate data in R. Most statistical applications work with (homogeneous) vectors, matrices or data frames. In order to give these types a natural \JSON representation, we can define certain special cases of \JSON structures which map to more simple R types. However currently there exist no formal guidelines, or even consensus between implementations on how R data should be represented in \JSON. Furthermore, upon closer inspection, even the most basic data structures in R actually do not perfectly map to their \JSON counterparts, and leave some ambiguity for edge cases. These problems have resulted in different behavior between implementations, and can lead to unexpected output for certain special cases. This paper explicitly describes a mapping between R classes and \JSON data, highlights problems and proposes conventions that can generalize the mapping to cover all common classes and cases in R. We emphasize the importance of type consistency when using \JSON to exchange dynamic data. The \texttt{jsonlite} R package is used as a reference implementation throughout the paper.}
...
@
 ...
 However even though it is possible to generate such \JSON, it is bad practice. Dynamic typing puts the responsibility of using consistent classes in the hands of the developer. Fields or collections with ambiguous object types are difficult to describe, interpret and process in the context of inter-system communication. When using \JSON to exchange dynamic data, it is important that each property and array is \emph{type consistent}. This means that the developer needs to make sure that properties are of the correct type before encoding to \JSON. It also means that \texttt{unnamed lists} in R should generally be avoided when designing interoperable structures. 
...
@
 ...
 This might seems obvious, but dynamic languages such as R can make it dangerously tempting to publish data containing mixed-type properties or collections. We already mentioned the example of other \JSON implementations which would encode atomic vectors either as \JSON primitive or \JSON array, depending on the length of the vector. Such inconsistent typing is very difficult for clients to predict and a likely source of nasty bugs. The use of consistent field types and  homogeneous \JSON arrays is a very strong convention among public \JSON APIs, for good reasons. We strongly recommend the R user to follow these conventions when generating \JSON data in R.
