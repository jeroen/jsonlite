...
A mapping between \JSON structures and several common R object classes is proposed that makes it natural to interface R objects through \JSON, and interface \JSON data through R objects.
 ...
 This document attempts to take away some of the ambiguity by explicitly describing the mapping between R classes and \JSON data, highlighting problems and propose conventions that can generalize the mapping to cover all common classes and cases in R.
