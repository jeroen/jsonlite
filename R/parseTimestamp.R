...
parseTimestamp.character <- function(x){
	...
	patterns <- list(
		ISODATE = "^(\\d{4})-(\\d{2})-(\\d{2})$",
		ISOTIME = "^(\\d{4})-(\\d{2})-(\\d{2})(T|\\s)(\\d{2}):(\\d{2})$",
		ISOTIMEUTC = "^(\\d{4})-(\\d{2})-(\\d{2})(T|\\s)(\\d{2}):(\\d{2})Z$",
		ISOTIMESECONDS = "^(\\d{4})-(\\d{2})-(\\d{2})(T|\\s)(\\d{2}):(\\d{2}):(\\d{2})$",
	...
	#select format
	outtime <- switch(datepattern,
		"ISODATE" = strptime(x, "%Y-%m-%d"),
		"ISOTIME" = strptime(x, "%Y-%m-%dT%H:%M"),
		"ISOTIMEUTC" = strptime(x, "%Y-%m-%dT%H:%MZ", tz="UTC"),
		"ISOTIMESECONDS" = strptime(x, "%Y-%m-%dT%H:%M:%S"),
		"ISOTIMESECONDSUTC" = strptime(x, "%Y-%m-%dT%H:%M:%SZ", tz="UTC"),
	);
